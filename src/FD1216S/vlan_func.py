...
#在服务模板下配置onu端口为tag模式
def ont_port_tag_profile(tn,Ont_Srvprofile_ID,User_Vlan):
    # 进入服务模板视图下
    tn.write('ont-srvprofile epon profile-id  {} '.format(Ont_Srvprofile_ID).encode() + b'\n')
    result = tn.read_until('OLT(config-epon-srvprofile-{})#  '.format(Ont_Srvprofile_ID).encode(), timeout=2)
    if result:
    ...
    #配置onu 端口1为tag 2000
    tn.write(('port vlan eth 1 %s'%User_Vlan).encode() + b'\n')
    tn.read_until('OLT(config-epon-srvprofile-{})#  '.format(Ont_Srvprofile_ID).encode(), timeout=2)
    tn.write((' port native-vlan eth 1 %s '%User_Vlan).encode() + b'\n')
    tn.read_until('OLT(config-epon-srvprofile-{})#  '.format(Ont_Srvprofile_ID).encode(), timeout=2)
    tn.write(b'show ont-srvprofile current' + b'\n')
    command_result =  tn.read_until('OLT(config-epon-srvprofile-{})#  '.format(Ont_Srvprofile_ID).encode(), timeout=2).decode("utf-8")
    print(command_result)
    ...
    '''
    if re.search(r"%s(\s+)%s(\s+)%s(\s+)-(\s+)%s(\s+)" % (User_Vlan,0,User_Vlan,User_Vlan), command_result):
    # if " 2000   0     2000   -     2000" in command_result :
        cdata_info("配置onu eth1 为tag %s成功"%User_Vlan)
        # 保存配置，退出服务模板
        tn.write(b'commit' + b'\n')
        tn.read_until('OLT(config-epon-srvprofile-{})#  '.format(Ont_Srvprofile_ID).encode(), timeout=2)
        tn.write(b'exit' + b'\n')
...
def ont_port_trunk_profile(tn,Ont_Srvprofile_ID,Ont_Port_ID,Vlan_list):
    ...
    # tn.write(b' port vlan eth 1 2000' + b'\n')
    # tn.read_until('OLT(config-epon-srvprofile-{})#  '.format(Ont_Srvprofile_ID).encode(), timeout=2)
    # # 配置onu端口1 为user-vlan 200转2001
    # tn.write(b'port vlan eth 1 2001' + b'\n')
    # tn.read_until('OLT(config-epon-srvprofile-{})#  '.format(Ont_Srvprofile_ID).encode(), timeout=2)
    # # 配置onu端口1 为user-vlan 300转2002
    # tn.write(b'port vlan eth 1 2002' + b'\n')
    # tn.read_until('OLT(config-epon-srvprofile-{})#  '.format(Ont_Srvprofile_ID).encode(), timeout=2)
    # # 配置onu端口1 为user-vlan 400转2003
    # tn.write(b'port vlan eth 1 2003' + b'\n')
    # tn.read_until('OLT(config-epon-srvprofile-{})#  '.format(Ont_Srvprofile_ID).encode(), timeout=2)
    # # 配置onu端口1 为user-vlan 500转2004
    # tn.write(b'port vlan eth 1 2004' + b'\n')
    # tn.read_until('OLT(config-epon-srvprofile-{})#  '.format(Ont_Srvprofile_ID).encode(), timeout=2)
    # # 配置onu端口1 为user-vlan 600转2005
    # tn.write(b'port vlan eth 1 2005' + b'\n')
    # tn.read_until('OLT(config-epon-srvprofile-{})#  '.format(Ont_Srvprofile_ID).encode(), timeout=2)
    # # 配置onu端口1 为user-vlan 700转2006
    # tn.write(b'port vlan eth 1 2006' + b'\n')
    # tn.read_until('OLT(config-epon-srvprofile-{})#  '.format(Ont_Srvprofile_ID).encode(), timeout=2)
    # # 配置onu端口1 为user-vlan 800转2007
    # tn.write(b'port vlan eth 1 2007' + b'\n')
    # tn.read_until('OLT(config-epon-srvprofile-{})#  '.format(Ont_Srvprofile_ID).encode(), timeout=2)
    # tn.write(b'show ont-srvprofile current' + b'\n')
    # time.sleep(0.1)
    # tn.write(b'\n')
    ...
    #     cdata_info("配置onu eth1 vlan为trunk2000-2007成功")
    #     # 保存配置，退出服务模板
    #     tn.write(b'commit' + b'\n')
    #     tn.read_until('OLT(config-epon-srvprofile-{})#  '.format(Ont_Srvprofile_ID).encode(), timeout=2)
    #     tn.write(b'exit' + b'\n')
    #     return True
    # else:
    #     cdata_error("配置onu eth1 vlan为trunk2000-2007失败")
    #     # 保存配置，退出服务模板
    #     tn.write(b'commit' + b'\n')
    #     tn.read_until('OLT(config-epon-srvprofile-{})#  '.format(Ont_Srvprofile_ID).encode(), timeout=2)
    #     tn.write(b'exit' + b'\n')
...
if __name__ == '__main__':
    ...
    ont_port_trunk_profile(tn, Ont_Srvprofile_ID='200', Ont_Port_ID='1', Vlan_list=[2000,2001])
