...
def check_port_static(port1_stats,port2_stats):
    ...
    if port1_stats.RxTotalFrames == 0 or port1_stats.TxTotalFrames == 0 or  port2_stats.TxTotalFrames==0:
        ...
        renix_error(
            '[test fail] [{}] rx packet ({}) or [{}] tx packets ({}) is 0 or [{}] tx packets ({})is 0'.
                format(port1,port1_stats.RxTotalFrames,port1,port1_stats.TxTotalFrames,port2,port2_stats.TxTotalFrames))
    ...
    if port2_stats.TxTotalFrames == 0 or port2_stats.RxTotalFrames == 0 or port1_stats.TxTotalFrames == 0:
        verdict = 'FAIL'
        renix_error(
            '[test fail] [{}] rx packet ({}) or [{}]tx packets ({}) is 0 or [{}] tx packets ({})'.format(port2,port2_stats.RxTotalFrames,port2,
                                                                                    port2_stats.TxTotalFrames,port1,port1_stats.TxTotalFrames))
...
def rate_test(port_location,down_stream_header,up_stream_header,rate=10,duration=10,stream_header=['Ethernet.ethernetII', 'VLAN.vlan', 'IPv4.ipv4','UDP.udp']):
    # 定义变量
...
def unicast_test(port_location,down_stream_header,up_stream_header,num=1,dataclassname=StreamBlockStats,stream_header=['Ethernet.ethernetII', 'VLAN.vlan', 'IPv4.ipv4','UDP.udp'],duration=10):
    # 定义变量, 'UDP.udp'
    sys_entry = get_sys_entry()
    verdict = 'PASS'
    errInfo = []
