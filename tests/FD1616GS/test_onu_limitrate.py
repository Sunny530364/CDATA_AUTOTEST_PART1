...
#         assert autofind_onu(tn, Gpon_PonID, Gpon_OnuID, Gpon_Gpon_SN)
#     with allure.step('步骤2:在OLT上通过Gpon_Gpon_SN的方式将ONU注册上线。'):
#         assert auth_by_sn(tn ,Gpon_PonID, Gpon_OnuID, Ont_Lineprofile_ID, Ont_Srvprofile_ID, Gpon_Gpon_SN)
#     with allure.step('步骤3:配置onu端口的入口的速率（traffic-profile ）'):
#         assert ont_port_limitrate_inbound(tn, traffic_profile_id=Traffic_Profile_ID_inbound,ponid=Gpon_PonID,ontid=Gpon_OnuID,ethid=Ont_Port_ID)
#     with allure.step('步骤4:添加虚端口vlan透传2000'):
#         assert add_service_port(tn, Gpon_PonID, Gpon_OnuID, Gemport_ID, Vlan_list)
#
# @pytest.fixture(scope='function')
# def port_outbound_limitrate_suit(login):
...
#         assert auth_by_sn(tn ,Gpon_PonID, Gpon_OnuID, Ont_Lineprofile_ID, Ont_Srvprofile_ID, Gpon_Gpon_SN)
#     with allure.step('步骤3:配置onu端口的入口的速率（traffic-profile ）'):
#         assert ont_port_limitrate_outbound(tn, traffic_profile_id=Traffic_Profile_ID_outbound,ponid=Gpon_PonID,ontid=Gpon_OnuID,ethid=Ont_Port_ID)
#     with allure.step('步骤4:添加虚端口vlan透传2000'):
#         assert add_service_port(tn, Gpon_PonID, Gpon_OnuID, Gemport_ID, Vlan_list)
#
# @pytest.fixture(scope='function')
...
def test_ont_port_outbound_limitrate(login):
    ...
    with allure.step('步骤5:打流测试。'):
        ...
        result_stats = rate_test(port_location=port_location, down_stream_header=down_stream_header,
                                    ...
                                    duration=duration )
        # if rate==10:
        #     #onu端口为1000M，上下行都发送端口速率的10%，也就是100M,onu端口下行限20M,所以下行比例是（20/100）=0.2， 上行是（100/100）=1
        #     # 判断port1的上下行流量是否都是不通的，如果是返回PASS，否则返回FAIL
        #     result1 = check_stream_rate(result_stats[0], result_stats[1],outbound_percent=0.2)
        # elif rate==100:
        #     # onu端口为100M，上下行都发送端口速率的100%，也就是下行1000M,上行100M,onu端口下行限20M,所以下行比例是（20/100）=0.2， 上行是（100/1000）=0.1
        #     # 判断port1的上下行流量是否都是不通的，如果是返回PASS，否则返回FAIL
        #     result1 = check_stream_rate(result_stats[0], result_stats[1], outbound_percent=0.2,inbound_percent=0.1)
        if port2_speed == 'SPEED_100M' and port1_speed == 'SPEED_100M':
            # onu端口为1000M，上下行都发送端口速率的100%，也就是100M,onu端口下行限10M,所以下行比例是（10/100）=0.1， 上行是（100/100）=1
            result1 = check_stream_rate(result_stats[0], result_stats[1], inbound_percent=1, outbound_percent=0.2)
        elif port2_speed == 'SPEED_100M' and port1_speed == 'SPEED_1G':
            # onu端口为1000M，上下行都发送端口速率的10%，也就是100M,onu端口下行限10M,所以下行比例是（10/100）=0.1， 上行是（100/100）=1
            result1 = check_stream_rate(result_stats[0], result_stats[1], inbound_percent=0.1, outbound_percent=0.2)
        elif port2_speed == 'SPEED_1G' and port1_speed == 'SPEED_100M':
            # onu端口为100M，上下行都发送端口速率的100%，也就是下行1000M,上行100M,onu端口下行限10M,所以下行比例是（10/1000）=0.01， 上行是（100/100）=1
            result1 = check_stream_rate(result_stats[0], result_stats[1], inbound_percent=1, outbound_percent=0.02)
        elif port2_speed == 'SPEED_1G' and port1_speed == 'SPEED_1G':
            # onu端口为100M，上下行都发送端口速率的100%，也就是下行1000M,上行100M,onu端口下行限10M,所以下行比例是（10/1000）=0.01， 上行是（100/100）=1
            result1 = check_stream_rate(result_stats[0], result_stats[1], inbound_percent=1, outbound_percent=0.02)
        # #删除onu端口的限速配置
...
if __name__ == '__main__':
    # for i in range(5):
    pytest.main(['-v',"-s",'test_onu_limitrate.py::test_dba_limitrate'])